...
export async function updateDomainData(domain: string, updatedSeconds: number | null, updatedFavIcon: string | null) {
    try {
        ...
        console.info(`Os dados do domínio ${domain} foram atualizados: ${JSON.stringify(domainData)}`)
    } catch(error) {
...
export async function checkDomainStored(incomingDomain: string): Promise<boolean> {
    try {
        let exists = false;
